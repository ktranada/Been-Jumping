...
var cl = ChunLi.Game.prototype = {
  ...
  create: function(){
    ...
    this.scoreText = game.add.text(10, 20, "",{
      font: "bold 16pt Arial"
    });

    game.add.text(670, 10, "'I win because I have a reason to fight.'", {
    ...
    // this.game.camera.focusOnXY(this.chunLi.sprite.x + 200, this.chunLi.sprite.y)
    game.camera.follow(this.chunLi.sprite);
  ...
  addPlatform: function(x, platformGroup, chunLi) {
    if ( x < game.width * 2) {
      ...
      var nextPlatPos = game.rnd.between(this.minPlatformGap, this.maxPlatformGap) + x;
  ...
  checkLanding: function(chunLi, platform){
    ...
    if (platform.y >= chunLi.y + chunLi.height/2) {
      ...
      if (this.chunLi.justSpawned) {
        if (firstRun){
          this.text = game.add.text(300,80, " Click & hold anyhwere to charge, \n then release to jump!", {
          ...
          this.text.shadowColor = "rgba(0,0,0,1)";
          firstRun = false;          
        }
      ...
      if (this.chunLi.boxJumping){
        var platformDiff = platform.platformNumber - this.chunLi.lastPlatform;
        if (platformDiff > 0){
